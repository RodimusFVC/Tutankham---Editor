...
class MapEditor:
    ...
    def on_map_click(self, event):
        ...
        col = int(canvas_x // (16 * self.zoom_level) * 0.5)
    ...
    def display_map(self):
        ...
        map_image = np.zeros((raw_map_layout.shape[0] * 16, raw_map_layout.shape[1] * 16, 4), dtype=np.uint8)
        ...
        if self.zoom_level != 1:
            ...
            new_width = int(map_image_rgb.shape[1] * self.zoom_level * 0.5)
        ...
        if self.show_grid.get():
            for x in range(0, int(map_width * 16 * self.zoom_level * 0.5), int(16 * self.zoom_level * 0.5)):
            ...
            for y in range(0, int(map_height * 8 * self.zoom_level), int(8 * self.zoom_level)):
                self.map_canvas.create_line(0, y, int(map_width * 16 * self.zoom_level * 0.5), y, 
                                           fill='#444444')
        
        self.map_canvas.configure(scrollregion=(0, 0, 
                                               int(map_width * 16 * self.zoom_level * 0.5), 
