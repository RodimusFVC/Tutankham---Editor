...
def load_maps():
    ...
    maps = []
    for map_index in range(num_maps):  			 # There are 4 maps
        start_offset = map_index * map_size  		 # Each map is 0x300 bytes
        map_layout = np.zeros((64, 12), dtype=np.uint8)  # 64 tiles wide, 12 tiles tall
        for i in range(12): 				 # Read 12 bytes for each row (64 tiles wide)
            map_layout[i] = map_data[start_offset + i * 12 : start_offset + (i + 1) * 12]
...
def rotate_map(map_data):
    ...
    return np.rot90(map_data, k=1)  # 90-degree counterclockwise rotation
...
def display_map(maps, all_tiles):
    ...
    for map_index, raw_map_layout in enumerate(maps):
        rotated_map = rotate_map(raw_map_layout)  # Correct map orientation
