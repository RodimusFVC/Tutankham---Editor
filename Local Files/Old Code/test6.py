...
# Apply the palette to the tile (convert indices to ARGB values)
def apply_palette_to_tile(tile_data, palette):
    height, width = tile_data.shape
    color_tile = np.zeros((height, width, 4), dtype=np.uint8)  # Create an ARGB tile

    print("Raw tile data:")
    print(tile_data)  # Print the raw tile data for debugging

    for y in range(height):
        for x in range(width):
            # Subtract 1, then divide by 16 and take the whole number result
            color_index = (tile_data[y, x] - 1) // 16

            # Check if the color index is within the palette range
            if 0 <= color_index < len(palette):
                # In the palette: (A, R, G, B) - correctly map the colors
                color_tile[y, x] = [palette[color_index][1],   # Red
                                     palette[color_index][2],  # Green
                                     palette[color_index][3],  # Blue
                                     palette[color_index][0]]  # Alpha
                # Debug print: Show the raw tile number and the selected color
                print(f"Raw tile value at ({y},{x}): {tile_data[y, x]} -> Palette index: {color_index} -> Color: {palette[color_index]}")
            else:
