...
#        row = int(canvas_y // (8 * self.zoom_level))
    ...
    def display_map(self):
        ...
        rotated_map = raw_map_layout
    
        map_image = np.zeros((rotated_map.shape[0] * 8, rotated_map.shape[1] * 16, 4), dtype=np.uint8)
        ...
        for row in range(rotated_map.shape[0]):
            for col in range(rotated_map.shape[1]):
                ...
                if tile_index != 0:
                    tile = self.all_tiles[tile_index - 1]  # <-- YOUR ORIGINAL HAD -1 HERE
                    color_tile = apply_palette_to_tile(tile, palette)
                    map_image[row * 8 : (row + 1) * 8, col * 16 : (col + 1) * 16, :] = color_tile

        map_image_rgb = np.flip(map_image, axis=2)  # <-- YOUR ORIGINAL HAD THIS
