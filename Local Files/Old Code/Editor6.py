...
class MapEditor:
    ...
    def find_teleporters(self, map_index):
        """Find all teleporter columns by scanning visual map for teleporter pattern"""
        visual_map = self.visual_maps[map_index]
        teleporter_cols = []
    
        for col in range(map_width):
            # Check for teleporter pattern (tiles 100, 38, 101 stacked vertically)
            for row in range(map_height - 2):
                if (visual_map[row, col] == 100 and 
                    visual_map[row + 1, col] == 38 and 
                    visual_map[row + 2, col] == 101):
                    teleporter_cols.append(col)
                    break  # Found one in this column, move to next column
