...
tile_roms = [
                 "./c1.1i",  # Tile Rom #1 - Contains 32 Tiles of 128 bytes each (16 * 16 // 2)
                 "./c2.2i",  # Tile Rom #2 - Contains 32 Tiles of 128 bytes each (16 * 16 // 2)
                 "./c3.3i",  # Tile Rom #3 - Contains 32 Tiles of 128 bytes each (16 * 16 // 2)
                 "./c4.4i",  # Tile Rom #4 - Contains 32 Tiles of 128 bytes each (16 * 16 // 2)
                 "./c5.5i"]  # Tile Rom #5 - Contains 32 Tiles of 128 bytes each (16 * 16 // 2)
visual_map_rom_path = "./c8.8i"  # Visual  Map ROM - Contains Maps 1, 2, 3, 4
logical_map_rom_paths = [
                 "./c6.6i",  # Logical Map ROM - Contains Maps 1, 2
                 "./c7.7i"]  # Logical Map ROM - Contains Maps 3, 4
object_rom_paths = [
                 "./m1.1h",  # Object Code ROM - Contains Difficulty 1 (Maps 1, 2, 3, 4) and Difficulty 2 (Maps 1, 2, 3, First Part of 4)
                 "./m2.2h"]  # Object Code ROM - Contains Difficulty 2 (Maps Second Part of 4) and Difficulty 3 & 4 (Maps 1, 2, 3, Part of 4)
...
CONFIG_BASE_OFFSET = 0x061E  # Config data starts 11 bytes before object data
...
WALL_TILES = [
    *range(0x00, 0x0F),      # 0-14
    0x11,                    # 17
    0x13,                    # 19
    *range(0x1F, 0x21),      # 31-32
    0x27, 0x28,              # 39-40
]

SPAWN_TILES = [
    0x17,                    # 23 - Player Respawn flame (Editor Representation Only!  Not Saved To Map/Code)
]
...
OBJECT_BASE_OFFSET = 0x0629  # Base Offset For Map 1 / Difficulty 1
OBJECT_BLOCK_SIZE  = 0x0148  # Current Known Block Size 
...
HIGH_SCORE_BASE_OFFSET = 0x04A0  # Base offset for high score data in m1.1h
HIGH_SCORE_ROM_PATH = "./m1.1h"  # High scores are in m1.1h
...
def load_high_scores():
    ...
    rom_data = read_rom(HIGH_SCORE_ROM_PATH)
    high_scores = []
    
    offset = HIGH_SCORE_BASE_OFFSET
    
    # Read HIGH SCORE (first entry)
    high_score = {
        'score': [rom_data[offset], rom_data[offset + 1], rom_data[offset + 2]],
        'name': '',
        'stage': 0
    }
    ...
    for i in range(NUM_HIGH_SCORES):
        score_bytes = [rom_data[offset], rom_data[offset + 1], rom_data[offset + 2]]
        name_bytes = [rom_data[offset + 3], rom_data[offset + 4], rom_data[offset + 5]]
    ...
    # Read stages (offset 0x4CD for 7 entries)
    stage_offset = 0x04CD
    for i in range(NUM_HIGH_SCORES):
        high_scores[i + 1]['stage'] = rom_data[stage_offset + i]
...
def save_high_scores(high_scores):
    ...
    for byte_val in high_scores[0]['score']:
        rom_data[offset] = byte_val
        offset += 1
    
    # Write 7 ranked entries (score + name)
    for i in range(NUM_HIGH_SCORES):
        entry = high_scores[i + 1]
        
        # Write score bytes
        for byte_val in entry['score']:
            rom_data[offset] = byte_val
            offset += 1
        ...
        for char in name_padded:
            rom_data[offset] = ord(char.upper())
            offset += 1
    
    # Write stages
    stage_offset = 0x04CD
    for i in range(NUM_HIGH_SCORES):
        rom_data[stage_offset + i] = high_scores[i + 1]['stage']
    
    # Write back to file
    with open(HIGH_SCORE_ROM_PATH, "wb") as f:
        f.write(rom_data)
...
class MapEditor:
    def __init__(self, visual_maps, logical_maps, all_tiles, palettes):
        ...
        self.root = tk.Tk()
        self.root.title("Tutankham Map Editor V10")
    ...
    def save_map_config(self, map_index, difficulty=0):
        ...
        # Write map pointers
        write_byte_to_roms(offset, (config['logical_map_ptr'] >> 8) & 0xFF, rom_data_cache)
        write_byte_to_roms(offset + 1, config['logical_map_ptr'] & 0xFF, rom_data_cache)
        write_byte_to_roms(offset + 2, (config['visual_map_ptr'] >> 8) & 0xFF, rom_data_cache)
        write_byte_to_roms(offset + 3, config['visual_map_ptr'] & 0xFF, rom_data_cache)
    
        # Write spawn rate and time limit
        write_byte_to_roms(offset + 4, config['spawn_rate'], rom_data_cache)
        write_byte_to_roms(offset + 5, config['time_limit'], rom_data_cache)
    
        # Write unknown bytes
        for i, byte_val in enumerate(config['unknown_bytes']):
            write_byte_to_roms(offset + 6 + i, byte_val, rom_data_cache)
    ...
    def rebuild_left_panel_for_data_editor(self):
        ...
        data_type_dropdown = ttk.Combobox(self.left_panel, textvariable=self.data_editor_type,
                                         values=["High Scores", "Palettes", "Map Config"],
                                         state="readonly", width=20)
    ...
    def update_data_editor_content(self):
        ...
        if data_type == "High Scores":
            ...
            ttk.Label(self.data_editor_content, text="Palette Editor", 
                     font=('Arial', 10)).pack(pady=20)
            ttk.Label(self.data_editor_content, text="Coming soon...", 
                     font=('Arial', 8)).pack()
        elif data_type == "Map Config":
            ttk.Label(self.data_editor_content, text="Map Config Editor", 
                     font=('Arial', 10)).pack(pady=20)
            ttk.Label(self.data_editor_content, text="Coming soon...", 
                     font=('Arial', 8)).pack()
    ...
    def display_data_editor(self):
        ...
        if data_type == "High Scores":
            ...
            self.map_canvas.create_text(400, 300, text="Palette Editor\nComing Soon", 
                                       font=('Arial', 24), fill='white')
        elif data_type == "Map Config":
            self.map_canvas.create_text(400, 300, text="Map Config Editor\nComing Soon", 
                                       font=('Arial', 24), fill='white')
    ...
    def mark_modified(self):
        self.modified = True
        self.root.title("Tutankham Map Editor V10 - *Unsaved*")
        ...
        for row in range(map_height):
            for col in range(map_width):
                ...
                if tile in SPAWN_TILES:  # 0x29 (player start) or 0x17 (respawn flame)
                    ...
                    self.map_canvas.create_rectangle(x, y, x+size, y+size,
                                               outline=color, width=2, tags='object_overlay')
                    self.map_canvas.create_text(x + size//2, y + size//2,
                                               fill=color, font=('Arial', int(6*self.zoom_level), 'bold'),
                                               tags='object_overlay')
    ...
    def save_file(self):
        try:
            ...
            messagebox.showinfo("Success", 
                              ...
                              f"High Scores: {HIGH_SCORE_ROM_PATH}\n"
    ...
    def create_window_icon(self):
        ...
        try:
            ...
            if tile_idx < len(self.all_tiles):
                ...
                icon_pil = Image.fromarray(icon_rgb, 'RGB')
