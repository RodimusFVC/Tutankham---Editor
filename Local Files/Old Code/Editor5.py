...
# Tile filter - only show tiles used for map building
TILE_WHITELIST = [
    # Wall tiles 0x00-0x0E (excluding 0x0F which is spawner)
    *range(0x00, 0x0F),  # 0-14
    # Skip 0x0F, 0x10 (spawner, keyhole animation)
    0x11,  # 17 - wall tile
    # Skip 0x12 (spawner)
    0x13,  # 19 - wall tile
    # Skip 0x14, 0x15 (animations, spawner)
    # Skip 0x16, 0x17 (keyhole/flame animations)
    # Skip 0x18 (spawner)
    # Skip 0x19, 0x1A (flame animations)
    # Skip 0x1B (spawner)
    # Skip 0x1C (flame animation)
    # Skip 0x1D (spawner)
    # Skip 0x1E (keyhole animation)
    # General walls
    *range(0x1F, 0x23),  # 31-34 (0x1F, 0x20, 0x21, 0x22)
    # Skip 0x23-0x25 (player sprites)
    # Path and walls
    0x26, 0x27, 0x28,  # 38, 39, 40
    # Skip 0x29-0x49 (sprites/animations)
    # Item boxes
    0x4A,  # 74 - treasure box (empty)
    # Skip 0x4B-0x61 (animations)
    0x62,  # 98 - treasure box (filled)
    # Skip 0x63 (explosion)
    # Skip 0x64-0x65 (teleporter pillars - composite object)
    # Skip 0x66-0x6E (animations)
    0x6F,  # 111 - ring in box
    0x70,  # 112 - key in box
    # Skip 0x71-0x72 (keyhole animations)
    # Skip 0x73-0x7B (door tiles - composite object)
    # Skip 0x7C-0x9F (remaining animations)
]
...
class MapEditor:
    def __init__(self, visual_maps, logical_maps, all_tiles, palettes):
        ...
        # Door tracking
        self.door_positions = {}  # {map_index: (row, col)}
    ...
    def find_door(self, map_index):
        """Find the door position on a map by scanning for the 3x3 pattern"""
        visual_map = self.visual_maps[map_index]
        
        for row in range(map_height - 2):
            for col in range(map_width - 2):
                # Check if this position contains the door pattern
                is_door = True
                for dr in range(3):
                    for dc in range(3):
    ...
    def is_door_tile(self, row, col):
        """Check if a tile position is part of the door"""
    ...
    def clear_door(self, row, col):
        """Clear door from specified position (replace with empty path)"""
    ...
    def place_door_at(self, row, col):
        """Place door at specified position"""
    ...
    def render_tile_palette(self):
        ...
        col_counter = 0
        for i in TILE_WHITELIST:  # Only render whitelisted tiles
            if i >= len(self.all_tiles):
                continue
                
            tile = self.all_tiles[i]
            ...
            self.tile_images.append((i, tile_photo))  # Store with actual tile index
            
            row = col_counter // tiles_per_row
            col = col_counter % tiles_per_row
        ...
        # Draw selection border for first tile
        ...
        # Set selected tile to first in whitelist
        if TILE_WHITELIST:
            self.selected_tile = TILE_WHITELIST[0]
    ...
    def on_tile_click(self, tile_index):
        ...
        # Find position in filtered palette
        if tile_index not in TILE_WHITELIST:
            return
            
        filtered_pos = TILE_WHITELIST.index(tile_index)
        tiles_per_row = 32
        tile_spacing = 4
        tile_display_size = int(16 * self.zoom_level)
        
        row = filtered_pos // tiles_per_row
        col = filtered_pos % tiles_per_row
        x = col * (tile_display_size + tile_spacing) + tile_spacing
        y = row * (tile_display_size + tile_spacing) + tile_spacing
        ...
        if 0 <= row < map_height and 0 <= col < map_width:
            ...
            if row + 3 <= map_height and col + 3 <= map_width:
                ...
                if self.place_door_at(row, col):
                    ...
                    # Restore old position if placement failed
