...
def analyze_special_bytes(logical_maps):
    """Analyze and report all special (non-standard) byte pairs in logical maps"""
    output = "=== SPECIAL BYTE ANALYSIS ===\n\n"
    total_special = 0
    
    for map_idx, logical_map in enumerate(logical_maps):
        output += f"MAP {map_idx + 1}:\n"
        special_tiles = []
        
        for row in range(64):
            for col in range(14):
                byte_pair = logical_map[row, col]
                # Check if it's NOT a standard pair
                if not (np.array_equal(byte_pair, [0x00, 0x00]) or 
                       np.array_equal(byte_pair, [0x55, 0x55]) or
                       np.array_equal(byte_pair, [0xCC, 0xCC])):
                    # Convert to visual coordinates (accounting for rotation and borders)
                    visual_col = row
                    visual_row = col - 1  # -1 to account for left border
                    special_tiles.append({
                        'visual_pos': (visual_row, visual_col),
                        'logical_pos': (row, col),
                        'bytes': (byte_pair[0], byte_pair[1])
                    })
                    total_special += 1
        
        if special_tiles:
            for tile in special_tiles:
                output += f"  Logical pos (row={tile['logical_pos'][0]}, col={tile['logical_pos'][1]}): "
                output += f"Bytes = {tile['bytes'][0]:02X} {tile['bytes'][1]:02X}\n"
        else:
            output += "  No special bytes found\n"
        output += "\n"
    
    output += f"Total special byte pairs found: {total_special}\n"
    print(output)
    
    # Also save to file
    with open("special_bytes_analysis.txt", "w") as f:
        f.write(output)
...
analyze_special_bytes(logical_maps)
