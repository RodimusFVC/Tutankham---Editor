...
#            color_index = (tile[y, x] - 1) // 16  # Adjusted color index mapping
            color_index = tile[y, x] % 16  # Adjusted color index mapping
...
class MapEditor:
    def __init__(self, maps, all_tiles, palettes):
        ...
        self.selected_tile = 0  # Start with tile 0 (0-based indexing)
        self.selected_map = 0
        self.tile_images = []
        self.zoom_level = 4.0  # Start at 4x zoom for better visibility
        self.modified = False
        
        self.root = tk.Tk()
        self.root.title("Tilemap Editor - Unsaved")
        self.root.geometry("1400x700")  # Set initial window size
        
        # Create BooleanVar after root window exists
    ...
    def setup_ui(self):
        ...
        self.map_canvas = tk.Canvas(canvas_frame, width=1024, height=96, bg='black')
        ...
        # Selected tile info
        self.tile_info_var = tk.StringVar(value="Selected: Tile 1")
        ttk.Label(palette_label_frame, textvariable=self.tile_info_var).pack(side=tk.LEFT, padx=20)
        
        # Tile palette canvas with scrollbar (horizontal)
    ...
    def render_tile_palette(self):
        ...
        tiles_per_row = 32  # Show 32 tiles per row for better layout
        tile_spacing = 4
        
        # Scale tiles with zoom level
        tile_display_width = int(8 * self.zoom_level)
        tile_display_height = int(16 * self.zoom_level)
        
        palette = self.palettes[self.selected_map]
        
        self.tile_images.clear()  # Clear old images
        
        for i, tile in enumerate(self.all_tiles):
            # Render the tile with current palette
...
#                                        scale_factor, axis=1)
            ...
            y_scale = int(self.zoom_level)
            x_scale = int(self.zoom_level * 0.5)  # Or 0.75, experiment
            color_tile_large = np.repeat(np.repeat(color_tile, y_scale, axis=0), x_scale, axis=1)
          
            # Convert RGBA to RGB for PIL (drop alpha channel, use RGB order)
            ...
            # Add border for tile 0 (selected initially)
            if i == 0:
    ...
    def on_tile_click(self, tile_index):
        ...
        self.selected_tile = tile_index  # Use 0-based indexing
...
#        tile_display_height = int(8 * self.zoom_level)
        tile_display_width = int(8 * self.zoom_level * 0.5)
        tile_display_height = int(16 * self.zoom_level)
...
#        row = int(canvas_y // (8 * self.zoom_level))
        col = int(canvas_x // (8 * self.zoom_level))
    ...
    def zoom_in(self):
        ...
        if self.zoom_level < 8:
            ...
            self.display_map()
            # Re-render tile palette at new zoom level
            self.palette_canvas.delete('all')
            self.render_tile_palette()
...
#        map_image = np.zeros((raw_map_layout.shape[0] * 8, raw_map_layout.shape[1] * 16, 4), dtype=np.uint8)
#        palette = self.palettes[self.selected_map]
        
#        for row in range(raw_map_layout.shape[0]):
#            for col in range(raw_map_layout.shape[1]):
#                tile_index = raw_map_layout[row, col]
#                # Use tile_index directly as array index (0-based indexing)
...
#                    map_image[row * 8 : (row + 1) * 8, col * 16 : (col + 1) * 16, :] = color_tile
        ...
        raw_map_layout = self.maps[self.selected_map]
        # Keep original dimensions - tiles are 8 wide × 16 tall after rotation
        map_image = np.zeros((raw_map_layout.shape[0] * 8, raw_map_layout.shape[1] * 16, 4), dtype=np.uint8)
        ...
        for row in range(raw_map_layout.shape[0]):
            for col in range(raw_map_layout.shape[1]):
                ...
                if tile_index < len(self.all_tiles):
                    tile = self.all_tiles[tile_index]
                    color_tile = apply_palette_to_tile(tile, palette)
                    # Tiles are 16 tall × 8 wide after rotation
                    color_tile_transposed = np.transpose(color_tile, (1, 0, 2))
                    map_image[row * 8 : (row + 1) * 8, col * 16 : (col + 1) * 16, :] = color_tile

        # Use same conversion as tile palette - just drop alpha, no flip
...
#            new_width = int(map_image_rgb.shape[1] * self.zoom_level)
            new_width = int(map_image_rgb.shape[1] * self.zoom_level * 0.5)  # Adjust width differently
            new_height = int(map_image_rgb.shape[0] * self.zoom_level)

            self.current_map_image = Image.fromarray(map_image_rgb, 'RGB').resize(
                (new_width, new_height), 
                Image.NEAREST  # Use nearest neighbor to keep pixels crisp
            )
        else:
        ...
        # Draw grid if enabled (accounting for zoom)
        if self.show_grid.get():
#            for x in range(0, int(map_width * 16 * self.zoom_level), int(16 * self.zoom_level)):
#                self.map_canvas.create_line(x, 0, x, int(map_height * 8 * self.zoom_level), 
#                                           fill='#444444')
#            for y in range(0, int(map_height * 8 * self.zoom_level), int(8 * self.zoom_level)):
...
#                                               int(map_height * 8 * self.zoom_level)))
             # Grid spacing based on actual tile dimensions × zoom
             tile_width_zoomed = int(8 * self.zoom_level)
             tile_height_zoomed = int(16 * self.zoom_level)
    
             # But the displayed width is scaled by 0.5 for aspect ratio
             display_tile_width = int(tile_width_zoomed * 0.5)
    
             # Vertical grid lines
             for col in range(map_width + 1):
                 x = col * display_tile_width
                 self.map_canvas.create_line(x, 0, x, map_height * tile_height_zoomed, fill='#444444')
    
             # Horizontal grid lines  
             for row in range(map_height + 1):
                 y = row * tile_height_zoomed
                 self.map_canvas.create_line(0, y, map_width * display_tile_width, y, fill='#444444')

        # Set scroll region to match displayed size
        self.map_canvas.configure(scrollregion=(0, 0, int(map_width * 8 * self.zoom_level * 0.5),
    ...
    def on_window_resize(self, event):
        ...
        if event.widget == self.root:
            ...
            # Check if size actually changed (avoid redundant redraws)
            if abs(new_width - self.last_width) > 10 or abs(new_height - self.last_height) > 10:
                self.last_width = new_width
                self.last_height = new_height
                # Redraw map to fit new window size
                self.display_map()
...
print(f"First tile shape after rotation: {all_tiles[0].shape}")
maps = load_maps()
