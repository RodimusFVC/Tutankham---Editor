...
def display_map(maps, all_tiles):
    ...
    for map_index, map_layout in enumerate(maps):
        ...
        map_image = np.zeros((map_layout.shape[0] * 16, map_layout.shape[1] * 8, 4), dtype=np.uint8)  # 16x8 per tile
        ...
        for row in range(map_layout.shape[0]):
            for col in range(map_layout.shape[1]):
                ...
                map_image[row*16:(row+1)*16, col*8:(col+1)*8] = all_tiles[tile_index]
