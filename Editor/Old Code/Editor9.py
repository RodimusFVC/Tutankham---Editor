...
class MapEditor:
    ...
    def render_tile_palette(self):
        ...
        columns = {
            'composite': {'x': tile_spacing, 'title': 'COMPOSITE', 'items': []},
            'walls': {'x': 250, 'title': 'WALLS', 'items': []},
            'spawns': {'x': 500, 'title': 'SPAWNS', 'items': []},
            'treasures': {'x': 700, 'title': 'TREASURES', 'items': []}
        }
        ...
        # Populate treasures (show empty and filled pairs)
        ...
        for col_key, col_data in columns.items():
            ...
            x_pos = x_start
            y_pos = tile_spacing
            col_width = 200  # Width available for each column
            max_y_in_row = 0
            ...
            for item in col_data['items']:
                ...
                if item['type'] == 'composite':
                    ...
                    label = f"0x{item['id']:02X}" if self.show_hex.get() else str(item['id'])
